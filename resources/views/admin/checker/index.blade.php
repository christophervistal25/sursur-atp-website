...
<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
  ...
  <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
      <a href="{{  route('checker.create') }}" class="button text-white bg-theme-1 shadow-md mr-2">Add New Checker</a>
      ...
      <tbody>
        @forelse($checkers as $checker)
            <tr>
              ...
              <td class="capitalize">{{ $checker->city ? $checker->city->name : '' }}</td>
              <td class="capitalize text-center">{{ $checker->created_at->format('m/d/Y') }}</td>
              <td class="text-center border-b w-5">
                <div class="flex sm:justify-center items-center ">
                    <a class="flex items-center bg-white rounded-full p-3 mr-3 shadow" href="{{ route('checker.edit', $checker->id) }}" title="Edit Checker"> 
