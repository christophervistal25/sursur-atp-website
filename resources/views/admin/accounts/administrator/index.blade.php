...
@section('page-title','List of Administrator')
@prepend('page-css')
@endprepend
@section('content')
<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
  ...
  <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
      <a href="{{  route('administrator.create') }}" class="button text-white bg-theme-1 shadow-md mr-2">Add New Administrator</a>
  ...
  <table class="table  display datatable w-full"  id="checkers-table">
      ...
      <tbody>
        @forelse($admins as $admin)
            <tr>
              <td>
                <div class="intro-x w-10 h-10 image-fit text-center">
                    <img alt="Image" class="rounded-full" src="{{ stage_asset('/storage/images/' . $admin->profile) }}">
                </div>
              </td>
              <td>{{ $admin->username }}</td>
              <td class="capitalize">{{ $admin->firstname }}</td>
              <td class="capitalize">{{ $admin->middlename }}</td>
              <td class="capitalize">{{ $admin->lastname }}</td>
              <td class="capitalize">{{ $admin->suffix }}</td>
              <td class="capitalize">{{  $admin->phone_number }}</td>
              <td class="capitalize">{{ $admin->created_at->format('m/d/Y h:i A') }}</td>
              <td class="text-center border-b w-5">
                <div class="flex sm:justify-center items-center ">
                    <a class="flex items-center bg-white rounded-full p-3 mr-3 shadow" href="{{ route('administrator.edit', $admin->id) }}" title="Edit Administrator"> 
          ...
          @empty
          <p class="text-theme-6">No Available data</p>
          @endforelse
      </tbody>
  </table>
</div>
