...
@section('page-title','List of Municipal')
@prepend('page-css')
@endprepend
@section('content')
<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
  <h2 class="text-lg font-medium mr-auto">
      List of Municipal Accounts
  </h2>
  <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
      <a href="{{  route('municipal-account.create') }}" class="button text-white bg-theme-1 shadow-md mr-2">Add New Municipal Account</a>
...
<div class="intro-y datatable-wrapper box p-5 mt-5">
  ...
  @endif
  <table class="table  display datatable w-full"  id="municipals-table">
      <thead>
        <tr>
            ...
            <tr>
              <td>
                <div class="intro-x w-10 h-10 image-fit text-center">
                    <img alt="Image" class="rounded-full" src="{{ stage_asset('/storage/images/no_image.png') }}">
                </div>
              </td>
              <td>{{ $account->username }}</td>
              <td class="capitalize">{{  $account->city->name }}</td>
              <td class="capitalize">{{  $account->phone_number }}</td>
              <td class="capitalize">{{ $account->created_at->format('m/d/Y h:i A') }}</td>
              <td class="text-center border-b w-5">
                <div class="flex sm:justify-center items-center ">
                    <a class="flex items-center bg-white rounded-full p-3 mr-3 shadow" href="{{ route('municipal-account.edit', $account->id) }}" title="Edit Municipal"> 
